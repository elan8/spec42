...
| --- | --- | --- |
| `version` | Action ref (`github.action_ref`, e.g. `v0.27.1`) | Spec42 release tag to install. Omit to use the `@vX.Y.Z` ref from `uses:`. |
...
- **Action ref vs CLI** — `uses: elan8/spec42@v0.27.1` selects the Action code at that tag; the CLI is downloaded for the same ref unless you override `version`. Keep them aligned.
