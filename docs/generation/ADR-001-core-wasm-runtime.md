...
Status: accepted for generator ABI 2.
...
The ABI has two imports under module `spec42`: a compact Postcard model-query call and diagnostics.
The guest exports its memory, allocation/deallocation functions, and a generation entrypoint that
returns its artifact list. Generator SDKs and language platform adapters may implement this
low-level boundary without changing the ABI.

The host compiles and pre-links the module before loading a model. Each invocation gets a fresh
Wasmtime store containing only the model view, diagnostics, resource limiter, and counters. The
engine may later cache compiled modules by SHA-256 digest, but no store or instance state may be
cached. No WASI implementation is linked.
...
- Postcard keeps structured model transfers and returned artifact lists compact.
