...
| **2** | Eliminate the dual pipeline: `server` depends on both `workspace` and `lsp_server`, which still ship parallel validation/workspace/LSP stacks (P1-2/P2-4/P2-9 above) | **Phase 1 landed 2026-07-02** — see `docs/engineering/TIER2-LSP-WORKSPACE-CONSOLIDATION.md`. Investigation found the low-level graph-building/diagnostics *are* already shared (`lsp_server` re-exports `workspace::semantic::*`/`sysml_model`); the real duplication is ~1200 lines of incremental-update orchestration in `lsp_server/src/workspace/services.rs`, which exists because `workspace`'s synchronous immutable-snapshot API can't express the live editor's async/cancellable relink needs. Added `WorkspaceSession` (`crates/workspace/src/session.rs`) generalizing `lsp_server`'s `coordinator.rs` token/generation state machine — as a **plain synchronous type**, not async, because `workspace` has an explicit, test-enforced guardrail (`tests/dependency_guardrails.rs`) against depending on `tokio`/`clap`/`axum`/`rmcp`/`tower-lsp`, keeping it protocol/runtime-neutral. Unused by any caller yet (zero behavior change). Phases 2-4 (migrate `lsp_server` onto it, then fold in the duplicated graph-update logic) not started — production LSP hot path, treat as its own multi-session initiative. |
