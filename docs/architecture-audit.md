...
| **2** | Eliminate the dual pipeline: `server` depends on both `workspace` and `lsp_server`, which still ship parallel validation/workspace/LSP stacks (P1-2/P2-4/P2-9 above) | **Scoped, design written 2026-07-02** — see `docs/engineering/TIER2-LSP-WORKSPACE-CONSOLIDATION.md`. Investigation found the low-level graph-building/diagnostics *are* already shared (`lsp_server` re-exports `workspace::semantic::*`/`sysml_model`); the real duplication is ~1200 lines of incremental-update orchestration in `lsp_server/src/workspace/services.rs`, which exists because `workspace`'s synchronous immutable-snapshot API can't express the live editor's async/cancellable relink needs. Proposed fix: add a `WorkspaceSession` type to `workspace` crate generalizing the token/generation/subscription pattern `lsp_server`'s `coordinator.rs` already proved in production, phased so `lsp_server` migrates onto it incrementally. Implementation not started — this is the production LSP hot path, treat as its own multi-session initiative. |
