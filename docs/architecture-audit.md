...
| **2** | Eliminate the dual pipeline: `server` depends on both `workspace` and `lsp_server`, which still ship parallel validation/workspace/LSP stacks (P1-2/P2-4/P2-9 above) | **Phases 1-2 landed 2026-07-02** — see `docs/engineering/TIER2-LSP-WORKSPACE-CONSOLIDATION.md`. Investigation found the low-level graph-building/diagnostics *are* already shared (`lsp_server` re-exports `workspace::semantic::*`/`sysml_model`); the real duplication is ~1200 lines of incremental-update orchestration in `lsp_server/src/workspace/services.rs`, which exists because `workspace`'s synchronous immutable-snapshot API can't express the live editor's async/cancellable relink needs. Phase 1 added `WorkspaceSession` (`crates/workspace/src/session.rs`) generalizing `lsp_server`'s `coordinator.rs` token/generation state machine — as a **plain synchronous type**, not async, because `workspace` has an explicit, test-enforced guardrail (`tests/dependency_guardrails.rs`) against depending on `tokio`/`clap`/`axum`/`rmcp`/`tower-lsp`, keeping it protocol/runtime-neutral. Phase 2 migrated `lsp_server`'s `SemanticCoordinator` to delegate to it — public API unchanged, all 8 call-site files untouched, `coordinator.rs` 191→176 lines, and the transition-validity logic now has one implementation (tested) instead of two (one previously untested). Phases 3-4 (fold the ~1200 lines of duplicated graph-update logic into `workspace`, then delete the dead code in `lsp_server`) not started — that's the large, riskier part touching the actual incremental-parse/relink logic on the production LSP hot path. |
