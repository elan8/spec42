...
**2026-07-15: the engine's own eagerly-computed `HostValidationReport` and `lsp_server`'s separate diagnostics-assembly implementation, both closed.** Two follow-ups discovered while re-verifying "is it really one pipeline now":

- `WorkspaceLoadRequest`'s default `ValidationTiming::Eager` meant every `load_workspace_snapshot` call (CLI/MCP/HTTP-API validation *and* diagram rendering) computed a full `HostValidationReport` that a repo-wide `grep` confirmed nothing in `crates/server` ever reads. Both call sites (`built_workspace.rs::validate_paths_with_semantics`, `host_snapshot.rs::load_snapshot_with_engine`) now pass `.with_validation_timing(ValidationTiming::Deferred)` — a one-line, behavior-neutral fix per site (`workspace`'s own `deferred_validation.rs` test already proved this path correct).
- The remaining real duplication: `workspace::snapshot::facts::collect_host_document_diagnostics` and `lsp_server::analysis::diagnostics_core::collect_document_diagnostics` independently assembled the same parse/untyped-part-usage/semantic/missing-library-context diagnostic set — `lsp_server`'s copy routed through a `check_providers: &[Arc<dyn SemanticCheckProvider>]` plugin list that, on investigation, had exactly one registrant (`DefaultSemanticChecks`) anywhere in the repo, ever, and served a `spec42-pro` composition model that was never built (per `internal_docs/PRO_ENABLEMENT_IMPLEMENTATION_PLAN.md`, still just a planning doc). New `sysml_model::collect_document_diagnostics` (`crates/sysml_model/src/semantic/diagnostics/document.rs`) is now the single implementation both crates call, including the parse-diagnostic assembly (previously a real, silent behavioral difference: `workspace`'s version defaulted a missing parser error code to `"parse_error"`; `lsp_server`'s left it `None` — now unified on `"parse_error"`, `SemanticDiagnostic.code` was never `Option` to begin with). `SemanticCheckProvider`/`CheckProvider`/`DefaultSemanticChecks` deleted; `Spec42Config.check_providers` field and `CapabilityProvider::check_providers()` removed. Removing that plugin field cascaded, mechanically (compiler-guided), into removing now-fully-unused `config: &Arc<Spec42Config>` threading through ~10 internal live-editing debounce/relink/evaluation helpers — stopped at the outermost `pub(crate)` LSP protocol handlers, where `config` stays for handler-signature consistency with siblings that do still need it.

Full suite green, clippy clean, manually verified both the batch path and the live-editing `publishDiagnostics` path (47 `lsp_integration` diagnostics tests exercising the real LSP notification flow) are unaffected in behavior.
