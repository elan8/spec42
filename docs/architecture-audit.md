...
**2026-07-14: P1-2 (dual pipeline) closed for the validation stack.** The state-machine half was already closed by Tier 2 (below): `SemanticCoordinator` deleted, folded into `workspace::WorkspaceSession`/`workspace_session`. The remaining piece — `lsp_server::validation` reimplementing its own DTOs, graph-projection walk, and disk-scan/build path instead of reusing `workspace`'s equivalents — is now closed too: `lsp_server::validation::{SemanticModelNode,SemanticModelRelationship,SemanticModelProjection}` deleted in favor of `workspace::{HostSemanticModelNode,HostSemanticModelRelationship,HostSemanticProjection}`; the hand-rolled `project_semantic_model` graph walk deleted in favor of `workspace::project_semantic_model`; and `lsp_server::validation::pipeline::validate_paths_with_semantics` (used only by `lsp_server`'s own tests) now shares `crates/lsp_server/src/validation/built_workspace.rs::semantic_report_from_built_workspace` with `crates/server/src/host_snapshot.rs` (the actual `spec42 check`/MCP/HTTP-API production path) instead of maintaining an independent implementation. `pipeline.rs` now only owns disk scanning (via `sysml_model::FileSystemDocumentProvider`); `built_workspace.rs` is the single place that turns a built graph into a validation report. This consolidation surfaced and fixed a real bug along the way: documents with a hard parse error were silently dropped from the document index (zero diagnostics instead of a parse error) — fixed once, in the shared function, rather than needing two independent fixes. The live-LSP-editing pipeline (`lsp_runtime`/`services.rs`) remains intentionally separate (incremental, different perf requirements).
