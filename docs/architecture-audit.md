...
| **2** | Eliminate the dual pipeline: `server` depends on both `workspace` and `lsp_server`, which still ship parallel validation/workspace/LSP stacks (P1-2/P2-4/P2-9 above) | **Phases 1, 2, 3a landed 2026-07-02** — see `docs/engineering/TIER2-LSP-WORKSPACE-CONSOLIDATION.md`. Investigation found the low-level graph-building/diagnostics *are* already shared (`lsp_server` re-exports `workspace::semantic::*`/`sysml_model`); the real duplication is ~1200 lines of incremental-update orchestration in `lsp_server/src/workspace/services.rs`, which exists because `workspace`'s synchronous immutable-snapshot API can't express the live editor's async/cancellable relink needs. Phase 1 added `WorkspaceSession` generalizing the token/generation state machine (plain synchronous type — `workspace` is guarded against depending on `tokio`). Phase 2 migrated `SemanticCoordinator` to delegate to it, public API unchanged. Phase 3 was split after reading `services.rs` in full showed the graph-update-logic merge is bigger than estimated (LSP-specific indexing concepts `workspace` doesn't have): **3a (done)** replaced the 5 manual `std::thread::spawn` sites with `rayon`, fixing a panic-swallowing bug along the way; **3b (rescoped, not started)** — folding the actual duplicated logic into `workspace` — needs its own design doc before any code moves. Phase 4 (delete resulting dead code) blocked on 3b. |
