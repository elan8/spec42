...
| exact dependency sets of the chain crates, the facade, `session_actor`, `workspace`, `lsp_server` (no `workspace`) and `server`; every downstream crate is a designated consumer; no async runtime in the chain; `walkdir`/`sha2`/`toml`/`zip`/`insta` are workspace-inherited | `crates/sysml_query/tests/architecture.rs` (cargo metadata and member-manifest scan) |
