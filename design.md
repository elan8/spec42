...
| checked-in semantic projections and normative evidence remain current, complete, and blocker-consistent | required CI runs `cargo snapshot check` and `cargo snapshot report --format json`; the standalone runner owns both contracts |
