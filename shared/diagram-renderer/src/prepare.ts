...
function prepareBrowser(visualization: VisualizationPayload): PreparedView {
...
function prepareGrid(visualization: VisualizationPayload): PreparedView {
...
function prepareGeometry(visualization: VisualizationPayload): PreparedView {
  const graphNodes = graphNodesForStandardView(visualization).filter((node) => !isSyntheticPackage(node));
  ...
  return {
    ...
    edges: graphEdges
      ...
      .filter((edge) => elements.some((node) => node.id === edge.source) && elements.some((node) => node.id === edge.target)),
