...
| -------------------------------------------------------- | -------------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `org.eclipse.elk.graph` (graph model)                    | `elk-core`                 | **Partial** | Core data model exists (nodes/ports/edges/labels/hierarchy/geometry) but is intentionally simplified vs ELK’s full `ElkNode/ElkEdge/ElkPort` + properties ecosystem. |
...
| `org.eclipse.elk.graph.json` (ELK JSON I/O)              | (planned `elk-graph-json`) | **Missing** | Key blocker for running ELK’s existing model corpus and doing automated differential testing.                                                                        |
...
| ------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| **Typed graph model**                 | **Implemented (simplified)** | `elk_core::Graph` supports nodes/ports/edges/labels/hierarchy/geometry.                                                               |
...
| **JSON I/O (ELK Graph JSON)**         | **Missing**                  | Planned as the next major milestone to unlock corpus-based parity work.                                                               |
