...
- **`ElementKind::is_definition()` added** — exhaustive, compiler-checked replacement for the `element_kind.as_str().ends_with(" def")` / `.contains("_def")` string-suffix heuristics scattered across `sysml_model` and `language_service`; all such call sites now use it.
