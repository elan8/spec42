...
use custom::{
    mark_sysml_model_parse_cached, software_analyze_workspace_result, software_project_view_result,
...
impl Backend {
    ...
    async fn software_visualization(
        &self,
        params: serde_json::Value,
    ) -> Result<dto::SoftwareVisualizationResultDto> {
        software_visualization_result(params)
    }

    async fn software_analyze_workspace(
        &self,
        params: serde_json::Value,
...
pub async fn run(config: Arc<Spec42Config>, server_name: &str) {
    ...
    .custom_method(
        ...
        Backend::software_analyze_workspace,
    )
