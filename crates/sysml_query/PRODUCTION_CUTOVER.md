...
| Type queries | Element identity, specialization scope, admitted libraries | Direct types, effective types with their origin, direct and transitive supertypes, direct subtypes, featuring type, and conformance as an explicit `Conforms`/`DoesNotConform`/`Indeterminate` outcome | **Producer landed, consumers pending.** `PublishedModel::types()` answers all of these from settled facts. `sysml_model`'s `specializes_transitively`/`feature_typing_conforms` and their consumers -- `sysml_diagnostics::checks::{kind_compatibility,structural_feature_conformance,view_metadata_conformance}`, `lsp_server::{lsp_runtime::features,views::feature_inspector}`, `workspace::snapshot::facts`, `generator_api::model` -- still run on the legacy graph and migrate with the diagnostics and views slices. |
