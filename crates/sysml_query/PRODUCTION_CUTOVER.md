...
`sysml_diagnostics` runs nine check modules over the mutable graph. The kind-compatibility and
structural-feature-conformance families are gone from it: the immutable publication owns them, and
their modules, helpers and kind tables are deleted rather than bridged.
