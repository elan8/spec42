...
(model
  (namespace
    (package 'Conditional Succession Example-2'
      ...
      (part_def 'Picture')
      (action_def 'Focus'
        (reference_usage in reference 'scene' : 'Conditional Succession Example-2::Scene'[part_def])
        (reference_usage out reference 'image' : 'Conditional Succession Example-2::Image'[part_def]))
      (action_def 'Shoot'
        (reference_usage in reference 'image' : 'Conditional Succession Example-2::Image'[part_def])
        (reference_usage out reference 'picture' : 'Conditional Succession Example-2::Picture'[part_def]))
      (action_def 'TakePicture'
        (reference_usage in reference 'scene' : 'Conditional Succession Example-2::Scene'[part_def])
        (reference_usage out reference 'picture' : 'Conditional Succession Example-2::Picture'[part_def]))
      (action_usage 'takePicture' : 'Conditional Succession Example-2::TakePicture'[action_def]
        (item_usage in 'scene')
        (item_usage out 'picture')
        (action_usage composite 'focus' : 'Conditional Succession Example-2::Focus'[action_def]
          (item_usage in 'scene'
            (feature_value (=)))
          (item_usage out 'image'))
        (if_action_usage)
        ...
        (action_usage composite 'shoot' : 'Conditional Succession Example-2::Shoot'[action_def]
          (item_usage in 'image')
          (item_usage out 'picture'
            (feature_value (=))))))))
~~~
