...
fn parse_outcome_strict_success_round_trips_through_the_real_store() {
    ...
    let result = sysml_resolution::syntax::parse_strict(src);
    assert!(result.is_ok());
...
fn parse_outcome_editor_recovery_retains_complete_diagnostics_on_a_warm_hit() {
    ...
    let cold = sysml_resolution::syntax::parse_for_editor(src);
    assert!(
...
fn parse_outcome_identical_content_at_a_different_path_hits() {
    ...
    let outcome = ParseOutcome::from_strict(&sysml_resolution::syntax::parse_strict(src));
