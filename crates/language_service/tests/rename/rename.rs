...
fn rename_cross_file_from_usage_with_long_identity_updates_both_files() {
    ...
    let def_content = "package Spec42MultiFileDefinitions { part def Spec42SmokeWidget; }";
    ...
    let pos = position_for_within(
        use_content,
        "part item : Spec42SmokeWidget",
        "Spec42SmokeWidget",
    );
    let edits = apply_rename(&workspace, use_path, pos, "Spec42RenamedSmokeWidget")
