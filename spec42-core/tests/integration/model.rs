...
fn lsp_sysml_model_general_view_graph_keeps_valid_office_subject_edge_without_synthetic_expansion() {
    ...
    let uri = "file:///C:/Git/sysml-examples/office/office.sysml";
    let content = std::fs::read_to_string("C:\\Git\\sysml-examples\\office\\office.sysml")
        .expect("read office example");
    ...
    let general_view_graph = &model_json["result"]["generalViewGraph"];
    let gv_nodes = general_view_graph["nodes"]
        .as_array()
        .expect("generalViewGraph nodes array");
    let gv_edges = general_view_graph["edges"]
        .as_array()
        .expect("generalViewGraph edges array");

    assert!(
        gv_edges.iter().any(|edge| {
            edge["type"].as_str() == Some("subject")
                && edge["source"].as_str() == Some("Office::goodComputer")
                && edge["target"].as_str() == Some("Office::Laptop")
        }),
        "expected Office generalViewGraph to include goodComputer -> Laptop subject edge, got: {gv_edges:#?}"
    );

    assert!(
        gv_nodes
            .iter()
            .any(|node| node["id"].as_str() == Some("Office::my_computer::laptop")),
        "expected explicit part usage nodes to remain in generalViewGraph, got nodes: {gv_nodes:#?}"
    );
...
#[test]
fn lsp_sysml_diagram_interconnection_view_office_example_is_not_empty() {
    let mut child = spawn_server();
    let mut stdin = child.stdin.take().expect("stdin");
    let mut stdout = child.stdout.take().expect("stdout");
    ...
    let interconnection = &diagram_json["result"]["scene"]["interconnectionView"];
    let selected_root = interconnection["selectedRoot"].as_str().unwrap_or("");
    let root_candidates = interconnection["rootCandidates"]
        .as_array()
        .cloned()
        .unwrap_or_default();
    let root_parts = interconnection["roots"][selected_root]["parts"]
        .as_array()
        .cloned()
        .unwrap_or_default();

    assert!(
        !selected_root.is_empty(),
        "expected office example to select an interconnection root, payload: {diagram_json:#?}"
    );
    assert!(
        !root_candidates.is_empty(),
        "expected office example to expose interconnection root candidates, payload: {diagram_json:#?}"
    );
    assert!(
        !root_parts.is_empty(),
        "expected office example to expose interconnection parts for selected root, payload: {diagram_json:#?}"
    );
