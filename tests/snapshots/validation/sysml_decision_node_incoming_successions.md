...
type=file
skip_validation=control nodes now publish as named declarations, but no semantic rule inspects the successions attached to one, so the canonical code decision_node_multiple_incoming does not exist yet
~~~
# SOURCE
~~~sysml
package Actions {
