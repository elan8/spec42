...
type=file
skip_validation=control nodes now publish as named declarations, but no semantic rule inspects the successions attached to one, so the canonical code decision_node_outgoing_multiplicity does not exist yet
~~~
# SOURCE
~~~sysml
package Actions {
